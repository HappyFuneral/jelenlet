<x-app-layout>
    ...
    <div class=" text-start px-6 py-4 whitespace-nowrap text-sm font-medium ">
        ...
        <a  href="{{route('attendanceModels.create')}}" type="button" class="items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 disabled:opacity-50 disabled:pointer-events-none dark:text-blue-500 dark:hover:text-blue-400 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">Create</a>
    </div>
    <div class="flex flex-col">
        <div class="m-1.5 overflow-x-auto">
            <div class="p-1.5 min-w-full inline-block align-middle">
                <div class="bg-white border rounded-sm divide-y divide-gray-200 dark:border-gray-600 dark:divide-gray-600 dark:bg-gray-700">
                    <div class="overflow-hidden">
                        <table class="table-auto min-w-full divide-y divide-gray-200 dark:divide-gray-600 dark:bg-gray-700">
                            ...
                            <tr>
                                ...
                                <x-th>
                                  ...
                                  <th scope="col" colspan="2" class="px-6 py-3 text-end text-xs font-medium text-gray-500 uppercase">Actions</th>
                            </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach(\App\Models\AttendanceModel::all() as $attendanceModel)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{$attendanceModel->name}}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{$attendanceModel->datetime}}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{$attendanceModel->type}}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{$attendanceModel->paid == 1 ? "Fizetve" : "Még nincs fizetve" }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{$attendanceModel->cause}}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                        <form action="{{route('attendanceModels.edit',$attendanceModel)}}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                        <form method="POST" action="{{route('attendanceModels.destroy',$attendanceModel)}}">
                                            @csrf
                                            @method('DELETE')
